...
The palette balances the professional weight of `#25343F` with the electric kinetic energy of `#FF9B51`. The system is now in `light` mode, reinforcing the high-contrast typography and sophisticated tonal layering.
...
- **Interactive Elevated Elements:** `surface-container-lowest` for a "lifted" feel or `primary-container` (#25343f) for high-impact focus areas.
...
- **Tonal Layering:** Instead of a shadow, place a `surface-container-lowest` card on a `surface-container` background. The subtle shift provides a sophisticated, "quiet" elevation.
...
- **Corners:** Always use `lg` (2rem) or `md` (1.5rem) rounding. The `roundedness` is set to `3` (maximum, pill-shaped), which aligns with this preference for significant rounding.
...
- **Embrace White Space:** Use the `24` (6rem) spacing token between major sections to let the design breathe. The `spacing` is set to `2` (normal), providing a good foundation for this.
